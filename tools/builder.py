...
def dataset_builder(args, config):
    ...
    if config.others.subset == 'train' and args.rate != 1.0:
        assert args.rate is not None
        subset_indices, _ = train_test_split(range(len(dataset)), train_size=args.rate, shuffle=False)
...
# The configurations of the optimizer and scheduler are adopted from ReCon (https://github.com/qizekun/ReCon.git)
